...
@view_config(route_name='consultar_asistencias', renderer='json', request_method='GET', permission='estudiante')
def consultar_asistencias_view(request):
    """
    Vista para consultar asistencias (solo estudiantes y administradores).
    """
    dbsession = request.dbsession
    usuario_id = authenticated_userid(request)
...
@view_config(route_name='consultar_asistencias_curso', renderer='json', request_method='GET', permission='admin')
